...
The word ``number'' in everyday English points at four different things. Only two of them are ordinary scalar numbers in the mathematical sense of this chapter.
...
In this first chapter, mathematical numbers are counts and measurements. The defining property: they participate in arithmetic and comparison. You can add them, subtract them, multiply them, compare them. Labels and positions use digits but do not participate the same way. Later chapters build vectors, matrices, tensors, and other structured objects out of numbers; do not confuse those later objects with the scalar numbers introduced here.
...
\]
A \textbf{set}, in the informal sense we use here, is just an unordered collection of distinct objects. The braces form the set. The symbol $\in$ denotes membership. ``$5 \in \mathbb{N}$'' reads ``$5$ is an element of $\mathbb{N}$''; it is true. ``$-1 \in \mathbb{N}$'' is false, because $-1$ is not in the collection.
...
\[
  A \;=\; \{x \in \mathbb{Q} \,:\, x > 0 \text{ and } x^2 < 2\}.
\]
Every element of $A$ is bounded above by $3/2$, because $(3/2)^2 = 9/4 > 2$. So $A$ has rational upper bounds. Does $A$ have a \emph{least} rational upper bound?

Here is the proof sketch. If a rational $b$ were the least upper bound of $A$, then $b^2$ would have to equal~$2$ exactly. Any $b$ with $b^2 < 2$ would not be an upper bound (a standard rational-perturbation argument finds a slightly larger rational still in $A$), and any $b$ with $b^2 > 2$ would not be \emph{least} (the same kind of argument finds a slightly smaller rational still above~$A$). So the least upper bound, if it exists as a rational, must satisfy $b^2 = 2$.
...
You cannot list the reals that way. The reason is not density alone; the rationals are dense too, yet they are listable. The formal reason is Cantor's diagonal argument: no enumeration, no matter how clever, captures every real number. A later chapter on sets and cardinalities proves this carefully. For now, remember the shape:
\begin{itemize}
...
A function can often be described in four styles. A working mathematician moves between them constantly. They are equivalent only when they specify the same input-output rule completely; a finite table by itself specifies only the listed values unless the domain is exactly those inputs.
...
No single representation is ``the'' function. The function is the rule; complete representations encode it. A short table is usually evidence about the rule, not the whole rule.
...
The interval $[0,\infty)$ means all real numbers $x$ with $x \geq 0$. The symbol $\subsetneq$ means ``proper subset'': contained in, but not equal to, the larger set.
...
$f : A \to B$ is \textbf{bijective} if it is both injective and surjective. A bijective function has an \textbf{inverse} $f^{-1} : B \to A$ satisfying $f^{-1}(f(a)) = a$ for every $a$ and $f(f^{-1}(b)) = b$ for every $b$.
\end{definition}
...
\textbf{Notation.} The product set $\mathbb{N} \times \mathbb{N}$ means the set of ordered pairs $(a,b)$ with $a \in \mathbb{N}$ and $b \in \mathbb{N}$.
...
\[
  C(n, k) \;=\; \frac{n!}{k!\,(n-k)!},
\]
where $n! = n \cdot (n-1) \cdot (n-2) \cdots 2 \cdot 1$ is the product of integers from $1$ up to $n$, with the convention $0! = 1$. Check: $C(5, 2) = 5!\,/\,(2!\,3!) = 120/(2 \cdot 6) = 120/12 = 10$. Matches the hand count.

\textbf{Step 6. (Optional bonus question.)} Is $C$ injective as a function $\{(n, k) : 0 \leq k \leq n\} \to \mathbb{N}$? No. $C(4, 1) = 4$ and $C(4, 3) = 4$: two distinct inputs, same output. So $C$ is not injective. Is it surjective onto $\mathbb{N}$? No under this chapter's convention $\mathbb{N} = \{0,1,2,\ldots\}$, because no binomial coefficient equals $0$. Every positive natural is hit, since $C(n,1)=n$ for every $n \geq 1$. If the codomain were $\mathbb{Z}^{+}$ instead of $\mathbb{N}$, the same rule would be surjective. This kind of question is exactly what Section~7 prepares you to answer.
...
\begin{itemize}
  ...
  \item Spivak, M.\ \textit{Calculus}, 4th ed.\ Publish or Perish, 2008.\ Chapters~1--2 --- the P1--P13 axiom framing for $\mathbb{R}$ and the least-upper-bound ``hole in $\mathbb{Q}$'' argument used in Section~3.
