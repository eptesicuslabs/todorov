...
## status: run2_slot_memory_retention_fixed complete (2026-04-15). val_bpb 1.4777, passkey 0/100 at 256 and 1024 (partial eval — pod stopped by user before passkey@4096 / selective_copy / structure_probe completed). FIVE paid runs all returning 0% passkey at 256 across two substrates (matrix and slot) and two retention regimes (broken inherited and fixed alpha_log_mean=5.0). diagnosis reframed: the language-modelling training objective on fineweb-edu does not exercise the memory substrate, so gradient descent collapses it to inactive (mean output gate stayed at 0.018 throughout the 4000-step run) and never learns to use it. cpu gates A and B prove the slot mechanism CAN retrieve when addresses are placed by hand. the architecture is not broken; it is being trained on a task that does not require what it provides. analysis at `neuroloc/wiki/synthesis/training_objective_vs_architectural_goal.md`. additional mistake documented at `wiki/mistakes/run2_slot_memory_fla_silent_fall_through.md`: first launch attempt of this run fell through to the python recurrent loop because `flash-linear-attention` was missing from the pod (now pinned in `requirements.txt` commit `edcfe5d`). next paid run must train on a corpus shaped to require memory: synthetic cognition curriculum, mixed pretraining, or two-phase pretraining. measurement bar is exceeding the ~2-3 bits-per-parameter knowledge ceiling of vanilla transformers (allen-zhu et al. 2024), not just nonzero passkey. run card at `neuroloc/wiki/tests/run2_slot_memory_retention_fixed_results.md`.
