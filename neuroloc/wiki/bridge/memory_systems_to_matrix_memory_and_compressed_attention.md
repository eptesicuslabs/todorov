# bridge: memory systems to matrix memory and compressed attention

status: current (as of 2026-04-23).

this article uses the project-native terms `matrix memory` and
`compressed attention`. external paper names are mentioned only when
...
linear attention of the outer-product-decay form implemented here as `matrix memory` follows the update rule of a classical Hopfield network: Hebbian outer-product storage with linear readout. capacity scales linearly with dimension (rank-limited), not exponentially.

## the current todorov implementation

### matrix memory (src/layers/kda.py) as working/episodic memory
...
### compressed attention (src/layers/mla.py) as retrieval/reference memory

compressed attention performs softmax attention over compressed per-token representations:
...
what compressed attention is NOT:
- it does not learn during inference (no weight modification, only cache growth)
- it has no consolidation mechanism (no transfer from compressed attention to anything)
- it does not interact with matrix-memory state (each layer's memory is private)

### the split

matrix memory forgets old associations to make room for new ones (fast, lossy). compressed attention retains everything but with compression (slow growth, exact retrieval). this produces functional complementarity:

- for recent tokens (within matrix memory's effective memory horizon of ~8-20 tokens), matrix memory provides O(1)-cost retrieval from the recurrent state. this is fast and efficient
- for distant tokens (beyond matrix memory's decay horizon), compressed attention provides O(T^2)-cost exact retrieval from the cache. this is expensive but precise
- the 75/25 split means most layers use the cheap, approximate memory (`matrix memory`), with a minority using expensive, exact memory (`compressed attention`)

## the proposed change

### a consolidation mechanism for todorov

if todorov were to implement CLS, it would need a consolidation mechanism: a process that transfers salient information from matrix memory's transient state to a more permanent store. three approaches:

**approach 1: periodic state distillation**

every N tokens, distill the current matrix-memory state into a "consolidated memory bank" stored alongside the compressed-attention cache. during compressed attention, attend over both the token cache and the consolidated state summaries. this adds a slow-growing episodic summary alongside the exact token cache.

    if t % N == 0:
        summary_t = compress(S_t)    [head_dim x head_dim -> d_c]
        memory_bank.append(summary_t)

risk: high. defining "compress" is non-trivial. the matrix-memory state is a head_dim x head_dim matrix; reducing it to a d_c vector requires choosing what information to preserve. the wrong compression throws away exactly the information that consolidation should preserve.

**approach 2: experience replay during training**

during training, periodically replay stored matrix-memory states from earlier in the sequence, using them as additional training signal. this is directly analogous to hippocampal replay: past states are reactivated and used to train the model.

risk: moderate. requires storing intermediate matrix-memory states (memory cost) and defining a replay objective (what loss function? reconstruction of the original input? prediction of future tokens?). the replay objective is not obvious.

**approach 3: matrix-memory state prediction loss**

add an auxiliary loss that trains compressed attention to predict what matrix memory's state would contain for a given context. this forces compressed attention to learn a compressed version of matrix memory's content, implementing a form of knowledge distillation from fast to slow system.

    L_consolidation = ||compressed_attention_output - detach(matrix_memory_output)||^2

risk: moderate. the compressed-attention output and matrix-memory output are not directly comparable (different layer positions, different representation spaces). would need careful alignment of the targets.
...
the strongest argument for consolidation: at very long context lengths (100K+ tokens), matrix memory's decay means early tokens are completely forgotten. a consolidation mechanism could preserve a compressed summary of the early context, extending effective memory beyond the decay horizon without the O(T^2) cost of full attention. but this is an engineering optimization, not a biological principle.

recommendation: do not implement consolidation for phase 5. the complexity-benefit tradeoff is unfavorable at current context lengths (2048-8192 tokens). revisit if/when todorov scales to 100K+ context, where matrix memory's decay becomes a genuine limitation rather than a feature.
...
1. **matrix memory is a linear associative memory** -- a classical Hopfield network with Hebbian writing and exponential decay. its capacity is rank-limited, its retrieval is linear, and it has no attractor dynamics. this is working memory, not episodic memory
2. **compressed attention is a modern Hopfield network** (via the Ramsauer et al. equivalence) with exponential capacity and one-step pattern completion. this is the mathematically powerful retrieval system
3. **the 75/25 split is an efficiency decision**, not a biological principle. matrix memory is O(1) per token, compressed attention is O(T^2). the split allocates most layers to the cheap system (`matrix memory`) and a minority to the expensive system (`compressed attention`), maximizing throughput
4. **the absence of consolidation is not a gap** at current scale. it would become one at very long context lengths where matrix memory's decay horizon becomes limiting
...
| matrix-memory state prediction loss | compressed attention learns matrix-memory abstractions | moderate (alignment problem) | no, defer |
| interference-based forgetting (delta rule) | better memory utilization | moderate (see [[plasticity_to_matrix_memory_delta_rule]]) | yes, phase 5b+ |
| attractor dynamics in matrix memory | error correction in retrieval | high (changes recurrence structure) | no, defer to phase 6+ |
