...
**MLA layers (6/24) are the closest analog to workspace broadcast**: MLA performs global attention (softmax over all past tokens), selectively retrieving information from the shared workspace based on query-key matching. this IS selective access to a shared representation. but MLA reads selectively -- it does not write selectively. and it operates on the inter-token dimension (selecting which past tokens to attend to), not the inter-layer dimension (selecting which features to broadcast).
