# bridge: biological attention to compressed attention

status: current (as of 2026-04-23).

this article uses the project-native term `compressed attention` for the
low-rank projected dot-product attention component. external paper names
such as "MLA" are mentioned only when referring to external literature.
...
### compressed attention (src/layers/mla.py) -- 3/24 layers (12.5%)
...
### matrix memory (src/layers/kda.py) -- 18/24 layers (75%)

matrix memory performs content-addressable retrieval from a matrix-valued recurrent state:
...
### what compressed attention computes vs what biological attention computes

compressed attention answers the question: "given the current token's query, which past tokens have the most relevant content?" this is INFORMATION RETRIEVAL -- finding relevant context in a memory store.
...
## the adversarial analysis: is compressed attention's 12.5% allocation biologically analogous?
...
compressed attention is 12.5% of layers (3/24). this is a minority of the total computation. the schedule (`matrix memory`, `matrix memory`, `matrix memory`, `mamba3`, `matrix memory`, `matrix memory`, `matrix memory`, `compressed attention`) x 3 means most processing is non-attentional (`matrix memory`).

surface reading: compressed attention is rare like attention is rare. both are minority systems.

### the deep disanalogy

this analogy fails on three axes.

**axis 1: level of sparsity.** biological attention is sparse at the ITEM level: out of all stimuli in the visual field, ~4 are attended. compressed attention is sparse at the LAYER level: out of 24 layers, 3 use softmax attention. but WITHIN each compressed-attention layer, attention is NOT sparse: every query attends to every key position with nonzero weight (full softmax). biological attention produces near-zero processing of unattended items. compressed attention produces nonzero influence from every past token. these are different kinds of sparsity operating at different levels.

**axis 2: modularity vs modulation.** compressed attention is a MODULE: 3 specific layers at fixed positions in the processing hierarchy. you can point to exactly which layers "do attention." biological attention is a MODULATION: any cortical area can be attention-modulated at any time. there is no fixed layer that "does attention" -- V1, V4, IT, PFC can all be attention-modulated depending on task demands. the architectural concept of "some layers are attention layers" has no biological counterpart.
...
### option 1: competitive attention in compressed attention (high complexity, moderate risk)

replace softmax with a competitive normalization mechanism in compressed-attention layers:

    scores = Q @ K^T / sqrt(d)
    competition = divisive_norm(scores, sigma=sigma, n=2)
    weights = competition / sum(competition)

where divisive_norm applies the Reynolds-Heeger normalization equation along the key dimension. this would introduce winner-take-all dynamics into compressed attention, making it more biologically analogous: a few tokens would receive most of the weight, and losing tokens would be actively suppressed (not just passively downweighted by softmax).

risk: divisive normalization has different gradient properties than softmax. the suppression mechanism may destabilize training. the effective sparsity may reduce compressed attention's ability to aggregate information from many sources, which is its primary computational role. this change could make compressed attention worse at its engineering function (information retrieval) while making it more biologically faithful.

assessment: do NOT implement. the engineering function of compressed attention (full retrieval from compressed cache) is well-validated at both 6M and 267M scale. making it more biologically plausible would change its computational role, and the 3:1 architecture assumes compressed attention provides something matrix memory cannot (exact long-range retrieval). introducing competition into compressed attention would make it more like a sparse retrieval mechanism, which matrix memory already provides. the result would be two matrix-memory-like systems, not complementary systems.

### option 2: attention-modulated beta gate in matrix memory (moderate complexity, low risk)

add a global "attention signal" that modulates beta across matrix-memory layers:
...
|---|---|---|---|---|---|
| competitive compressed attention | high | high | high | negative (degrades retrieval) | do NOT implement |
...
the most honest assessment: the word "attention" in compressed attention and the word "attention" in neuroscience refer to different computations. compressed attention should be evaluated on its engineering merits (does it improve language modeling?), not on its biological fidelity. the bridge between biological attention and todorov is not through compressed attention but through matrix-memory gating mechanisms (beta, alpha) and through the training objective (cross-entropy as implicit precision weighting).
