# bridge: synaptic plasticity to matrix-memory delta rule

status: current (as of 2026-04-23).

this article uses the project-native term `matrix memory` for the
outer-product decayed recurrent memory component. `delta rule` remains a
mathematical operation name, not a paper-brand reference.
...
the current matrix-memory update (`src/layers/kda.py`, lines 190-209):
...
- biological analog: this is the classical Hebbian outer product rule, identical in form to the Hopfield storage prescription. it is the most direct biological correspondence in the entire matrix-memory mechanism
...
- no explicit erasure: the delta rule in its original formulation (Schlag et al. 2021) includes an erasure term that removes old key-value associations before writing new ones. matrix memory relies on alpha decay for forgetting, which is indiscriminate (decays everything equally) rather than targeted

## is matrix memory a form of STDP?

**no.** the analysis must be precise:

1. STDP requires timing dependence between PRE and POST activity at DIFFERENT times. the sign and magnitude of Delta_w depend on t_post - t_pre. matrix memory's k_t * v_t^T uses k and v from the SAME timestep t. there is no timing window.
...
3. STDP is a LEARNING rule (it changes the parameters of the system). the matrix-memory state update is a COMPUTATION rule (it computes the state as a function of the input and previous state). the parameters of the matrix-memory layer (alpha_log, beta_proj, q/k/v projections) are learned by backpropagation, not by the state update itself.
...
biological plasticity suggests three modifications that matrix memory currently lacks:
...
in the matrix-memory layer's recurrent update path (`src/layers/kda.py`, lines 190-209):
...
### overall assessment
these three modifications bring matrix memory closer to biological plasticity principles while preserving its core strengths (fast Hebbian writing, content-addressable retrieval, efficient recurrence). the BCM-like alpha is the lowest-risk change and should be tested first. targeted erasure is the highest-impact change for memory-intensive tasks. homeostatic scaling is a safety mechanism for long-context training stability.
...
## does the absence of STDP in matrix memory limit what todorov can learn?
...
**possibly for temporal pattern learning.** STDP excels at learning temporal sequences and detecting causal relationships between events separated by short time delays. matrix memory's outer-product association is instantaneous (k_t and v_t from the same timestep), so it cannot directly learn "if pattern A occurs 5 tokens before pattern B, strengthen the association." it CAN learn this indirectly through the recurrence (the decayed state carries information about pattern A when pattern B arrives), but the learning rule itself does not explicitly encode timing dependence.
