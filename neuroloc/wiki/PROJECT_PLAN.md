...
last updated: 2026-04-12 (phase B simulation round complete on partial sweep, asymmetric capacity ceiling is the load-bearing finding, awaiting user direction)
...
**immediate next action**: phase A and phase B both complete on partial sweeps (results captured in `wiki/tests/encoding_simulation_round_a.md` and `wiki/tests/encoding_simulation_round_b.md`). round A on symmetric outer-product memory said sign-only encoding wins. round B on the project's actual asymmetric matrix memory at d_head=64 reverses the leader: tanh-bounded continuous encoding leads at 16-48 pattern counts, sign-only is a strong second only at 8-16 patterns, and **no encoding tested produces meaningful recall (mean cosine > 0.5) above 8 stored patterns**. the only encoding to cross 0.5 cosine at any tested condition is dense-key plus sparse-value at 8 patterns (0.51). the trend is monotonically downward: every encoding falls below 0.2 mean cosine by 32 patterns. this is a capacity-ceiling finding, not an encoding finding. the leading interpretation is that the matrix memory at d_head=64 has a hard rank-related capacity ceiling well below the project's training operating point (sequence length 2048, ~thousands of patterns in flight). round B's pattern-count sweep stopped at 48 because the local Windows MINGW-W64 numpy segfaults at pattern_count >= 64; the rank-ceiling test (pattern_count = 64 = head_dim) is deferred to a re-run on a linux pod. **next**: user direction is needed. four candidate moves are listed in `wiki/tests/encoding_simulation_round_b.md`: (a) enlarge the head dimension, (b) route long-range retrieval through compressed attention while the matrix memory handles short-range only, (c) test the erasure-toggle-on cells of the existing round-B data, (d) replace the matrix memory entirely with a non-outer-product mechanism. no GPU training code should change until the user picks among these.
...
- **2026-04-12** — phase A simulation round complete. three existing simulations (`memory/capacity_scaling`, `prototypes/rate_coded_spike`, `sparse_coding/hierarchical_ternary`) ran on CPU, producing metrics that consistently rank sign-only encoding above three-level magnitude quantization on symmetric outer-product memory recall. continuous bounded encoding produces high cosine similarity but zero exact recall. zeros in the encoded vector destroy the address space. the gap to the project's architecture (asymmetric, single-step, decayed, optionally-erased) is identified and documented in `wiki/tests/encoding_simulation_round_a.md`. round B (asymmetric simulation) is the next deliverable.
- **2026-04-12** — phase B simulation round complete on partial pattern-count sweep [8, 16, 32, 48]. the new simulation `simulations/memory/asymmetric_outer_product_recall.py` implements the project's actual matrix memory operation (asymmetric, single-step, decayed, optional erasure) at head dimension 64 and tests seven candidate input encoding configurations. 24 trials per cell, 12,096 trial records. **leading finding: at head dim 64, no encoding tested produces meaningful recall above 8 patterns. the only crossing of 0.5 mean cosine recall is dense-key plus sparse-value at 8 patterns (0.51).** at 32 patterns the leader is tanh-bounded continuous encoding at cosine 0.19 and top-1 accuracy 0.21 (random chance is 0.031). the encoding leader REVERSED from round A: round A's sign-only winner is now the second-best at low loads and degrades as fast as the others above 16 patterns. the result is a capacity-ceiling finding, not an encoding finding. the rank-ceiling test (pattern_count = 64 = head_dim) is deferred because the local Windows MINGW-W64 numpy segfaults at that pattern count. four candidate next moves are listed in `wiki/tests/encoding_simulation_round_b.md`: enlarge head dim, route long-range through compressed attention, test the erasure-on cells, replace the matrix memory entirely. user direction is needed before any GPU code changes.
